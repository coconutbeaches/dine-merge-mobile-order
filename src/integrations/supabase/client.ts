...
if (SUPABASE_URL && SUPABASE_PUBLISHABLE_KEY) {
  ...
  supabaseClient = createBrowserClient<Database>(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, {
    auth: {
      ...
      flowType: 'pkce',
      // Use processLock (per-tab) instead of the default navigator.locks
      // (cross-tab). navigator.locks deadlocks if any holder context
      // terminates without releasing — acquireTimeout is -1 inside auth-js,
      // so every subsequent getSession/getUser hangs forever, which cascades
      // into every Supabase query blocking on the Authorization header.
      // Cross-tab serialization just dedupes refreshes in multi-tab use —
      // not worth the total-deadlock failure mode.
      lock: processLock,
    },
    realtime: {
      connect: true,
