-- Update orders_by_day_and_guest_type function to fix date range and improve guest type handling
...
AS $$
  SELECT
    ...
    COUNT(CASE WHEN p.customer_type = 'hotel_guest' THEN 1 END) AS hotel_guest_orders,
    COALESCE(SUM(CASE WHEN p.customer_type = 'hotel_guest' THEN o.total_amount ELSE 0 END), 0) AS hotel_guest_revenue,
    COUNT(CASE WHEN p.customer_type != 'hotel_guest' OR p.customer_type IS NULL THEN 1 END) AS outside_guest_orders,
    COALESCE(SUM(CASE WHEN p.customer_type != 'hotel_guest' OR p.customer_type IS NULL THEN o.total_amount ELSE 0 END), 0) AS outside_guest_revenue
