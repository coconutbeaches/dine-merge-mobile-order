...
async function bindRestaurantHandshakeToSession(
  ...
  if (!normalizeRestaurantServiceLocation(handshake.table_number)) {
    return { ok: false, error: 'WhatsApp verification is not for a restaurant order' };
  }
  ...
  if (updateError) {
    console.error('[api/guest/register] Restaurant handshake bind failed:', updateError.message);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (payload && typeof payload === 'object') {
      ...
      restaurantLocation = normalizeRestaurantServiceLocation(tableNumber);
      const handshakeDecisionIsAuthoritative = Boolean(restaurantLocation);
  ...
  if (restaurantLocation) {
    const binding = await bindRestaurantHandshakeToSession(
      serviceClient,
